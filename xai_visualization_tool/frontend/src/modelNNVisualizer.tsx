...
const ModelNetworkVisualizer: React.FC<ModelNetworkVisualizerProps> = ({
    ...
    useEffect(() => {
        ...
        const connectionsData = weightSource === 'model' ? weight_matrices :
            (node_node_matrices || weight_matrices);

        const weightRange = weightSource === 'model' ? visualizationData.weight_range :
            (visualizationData.node_node_range || visualizationData.weight_range);

        const connections = createConnections(model_structure, connectionsData, layerSpacing, height, neuronSpacingFactor);
    ...
    }, [visualizationData, firstLayerData, connectionFilter, weightSource]);
        ...
        if (layerIndex < model_structure.length - 1) {
            ...
            for (let i = 0; i < layer.neurons; i++) {
                for (let j = 0; j < nextLayer.neurons; j++) {
                    ...
                    console.log(`Extracting weight for source neuron ${i}, target neuron ${j}:`, {
                        rawWeight,
    ...
    try {
        ...
        if (hasNodeNode) {
            ...
            weightSourceOptions.forEach(({x, label, type}) => {
                ...
                const sourceGroup = weightSourceGroup.append('g')
        ...
        legend.append('g')
            ...
            .attr('fill', 'black')
            .attr('text-anchor', 'middle')
            .text(`${weightSource === 'model' ? 'Model' : 'Node-Node'} Connection Weights`);
